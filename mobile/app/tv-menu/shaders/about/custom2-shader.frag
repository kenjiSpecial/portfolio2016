...
void main(){
    ...
    if(uState == 0.){
        ...
        float remind = mod( vUv.y*tileNum + uTime, 1.0 );
        float remindX = mod( vUv.x*tileNum + uTime, 1.0 );
        float xPos = remind; //(vUv.x - uv2.x) * tileNum * remind;
        float yPos = remind; //(vUv.y - uv2.y) * tileNum;

        colR = texture2D( texture1, vec2( xPos, yPos )).r;
        colG = texture2D( texture1, vec2( xPos + 0.02 * cos(uTime * 3.) * rate, yPos )).g;
        colB = texture2D( texture1, vec2( xPos - 0.02 * cos(uTime * 1.) * rate, yPos )).b;
        ...
        colR = texture2D( texture1, vec2( xPos, yPos )).r;
        colG = texture2D( texture1, vec2( xPos + 0.02 * cos(uTime * 3.), yPos )).g;
        colB = texture2D( texture1, vec2( xPos - 0.02 * cos(uTime * 1.), yPos )).b;
    ...
    n = snoise(vec2( vUv.x * 600.  *cos(uTime), vUv.y * 600. *sin(uTime)  ) );
        ...
        colG = texture2D( texture1, vec2( xxPos + 0.02 * cos(uTime * 3.) * (1.-rate), yyPos )).g;
        colB = texture2D( texture1, vec2( xxPos - 0.02 * cos(uTime * 1.) * (1.-rate), yyPos )).b;
