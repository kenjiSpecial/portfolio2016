...
vec3 getCol(float XPos, float rate){
    ...
    float yPos;
    float yPos0 = 0.333 + cos(uTime * 2.0) * 0.08 * (1.-uRate);//15;
    float yPos1 = 0.666 - cos(uTime * 2.0) * 0.06 * (1.-uRate);
    ...
    float xOutPos = clamp(XPos, 0., 1. - uRate * 0.45);

    return texture2D( texture, vec2( xOutPos, yPos )).rgb;
}
...
void main(){
    ...
    if(uState == 0.0){
        ...
        if(uState <= 0.1){
            ...
            texel.r = clamp(texel.r + 0.5, 0., 1.);
            texel.g = clamp(texel.g + 0.5, 0., 1.);
            texel.b = clamp(texel.b + 0.5, 0., 1.);
            ...
            texel.r = clamp(texel.r + 0.5 - 0.3 * rate, 0., 1.);
            texel.g = clamp(texel.g + 0.5 - 0.3 * rate, 0., 1.);
            texel.b = clamp(texel.b + 0.5 - 0.3 * rate, 0., 1.);
            float colR = (texel.r * n);
            float colG = (texel.g * n);
            float colB = (texel.b * n);
            ...
            texel.g = clamp(texel.g * 1.2, 0., 1.);
            texel.r = clamp(texel.r * 1.2, 0., 1.);
            texel.b = clamp(texel.b * 1.2, 0., 1.);
            float colR = (texel.r * n);
            float colG = (texel.g * n);
            float colB = (texel.b * n);
